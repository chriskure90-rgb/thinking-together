...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Task Selection: return JSON so the client can extract structured lesson plan fields
    if (stepId === 'task-selection') {
      ...
      if (parsed?.message && parsed?.lessonPlan) {
        return res.json({ text: parsed.message, lessonPlanUpdate: parsed.lessonPlan })
      }
